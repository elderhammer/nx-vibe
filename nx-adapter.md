...
| D-适配-3 | 重建载体 | **prj 另存副本**（几何同源、零 STEP 导入风险）——2026-09-03 M4c 实证补注：**STEP 直开载体闭环达标**（OpenDisplay 隐式翻译 + CreateCamSetup，见 nx-journal-manual-verification.md M4c 节），M3 归零在 STEP 载体上同款复现 → **副本口径解除，降级为调试辅助载体** |
...
| M4 | 几何维度 + STEP 跨件 | ✅ 几何维度**定案 known-skip**（2026-09-03 写探针 v1-v5 实证：2.5D 域几何不裸化，工序级/组级全空——实证链见核对清单 M4 节）；✅ **M4c STEP 跨件闭环归零复现**（2026-09-03 GUI 实测：STEP 直开载体重建，报告与 M3 副本口径逐字段一致——structure 15/15、tool 65/65、parameter 98/98、strategy 55+7 known_skip、mcs 1/1、deviations 7 同款豁免） |
